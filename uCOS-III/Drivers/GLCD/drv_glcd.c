...
#define GLCD_BOOSTER_ON_DLY                1200     // 120ms
#define DLY_100US                          2000     // 100us
 ...
 * Parameters: GLCD_SendCmd Cmd, uint8_t pData, uint32_t Size
...
uint32_t Contrast;
  ...
  if(Write2_DRAM)
  {
    ...
    if(Cmd != RAMWR)
    {
      ...
      {
        // Flush pixel
        GLCD_SPI_TransferByte(PixelHold);
      }
      // Release CS
    ...
    else
    {
      ...
      while(Size > 1)
      {
        ...
        else
        {
          ...
          GLCD_SPI_SendBlock((uint8_t *)&PixelHold,3);
          PixelHold = 0;
        }
	...
	if(Glcd_Iss==GLCD_ISS_0)
	{
      ...
      DataMode.MY = pDATCTR_Data->MY;
			...
			GLCD_SendCmd(MADCTR,(uint8_t *)&DataMode,0);
      // 4K colors Type A
			ColourSch = 3;
    ...
    case SETCON:
      ...
      return(GLCD_SendCmd(VOLCTR,(uint8_t *)&Contrast,0));
    case VOLCTR:
      ResRatio = *(pData+1);
      break;
	default:
  ...
  GLCD_SPI_TransferByte(pGLCD_CmdCtrl->Cmd);
  if(Cmd == RAMWR)
  {
  ...
  if(pGLCD_CmdCtrl->Dir == GLCD_READ)
  {
    if(pGLCD_CmdCtrl->Pulse)
    {
      if(Size)
      {
        *pData++ = GLCD_SPI_TransferByte(0xFFFF);
        --Size;
      }
    }
...
pDATCTR_Data_t pDATCTR_Data;
  ...
  // Rest L > 10us H
  GLCD_SetReset(0);
  for(volatile uint32_t i = GLCD_DLY_10US; i ; --i);
  GLCD_SetReset(1);
  // Wait 5 ms
  DelayResolution100us(GLCD_RESET_DLY);
  // Identify LCD and command interpreter
  Glcd_Iss = GLCD_ISS_0;
  GLCD_SendCmd(RDDID,(uint8_t *)&Data,0);
  ...
  if(Glcd_Iss == GLCD_ISS_1)
  {
    ...
    assert(GLCD_DEF_DPL_ML == 0);
	  ...
	  GLCD_SendCmd(DISCTR,(uint8_t *)&Data,0);
	  //  OSCON (Oscillator ON)
	  GLCD_SendCmd(OSCON,NULL,0);
    // Set contrast
    Data = 0x26 | (0x0UL << 8);
    GLCD_SendCmd(VOLCTR,(uint8_t *)&Data,0);
  }
  else
  {
    // Set contrast
    Data = (Glcd_Iss == GLCD_ISS_0_TYPE1)?0x3F:0x30;
    GLCD_SendCmd(SETCON,(uint8_t *)&Data,0);
  }
...
#error "PARTIAL_MODE not supported yet"
  ...
  GLCD_SendCmd(DATCTR,(uint8_t *)&Data,0);
  // Display Data Write & Display On
  // COLUMN ADDRESS SET and PAGE ADDRESS SET
  Data = 0 | ((GLCD_HORIZONTAL_SIZE - 1)<<8);
  GLCD_SendCmd(RASET,(uint8_t *)&Data,0);
  GLCD_SendCmd(CASET,(uint8_t *)&Data,0);
  ...
  if(Glcd_Iss == GLCD_ISS_1)
  {
    // PWRCTR
    Data = 0x1F;
    GLCD_SendCmd(PWRCTR,(uint8_t *)&Data,0);
  }
 ...
 * Return: none
 *
 * Description: Clear display function
 *
 *************************************************************************/
void GLCD_ClearDisplay (void)
{
    GLCD_putchar('\f'); // Page Break
}
